...
<script type="text/javascript">
	$(function(){
		$("#warna").load("<?=site_url("master/variasi?load=warna")?>");
		$("#size").load("<?=site_url("master/variasi?load=size")?>");
		...
		$("#simpan").on("submit",function(e){
			...
			$.post("<?=site_url("master/tambahvariasi")?>",datar,function(e){
				var data = eval("("+e+")");
	...
	function refreshWarna(page){
		$("#warna").load("<?=site_url("master/variasi?load=warna&page=")?>"+page);
	}
	function refreshSize(page){
		$("#size").load("<?=site_url("master/variasi?load=size&page=")?>"+page);
	}
	...
	function hapusSize(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("master/hapussize")?>",{"theid":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("master/variasi")?>";
						});
					}else{
	...
	function hapusWarna(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("master/hapuswarna")?>",{"theid":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
...
<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="modalLagu" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<form id="simpan" action="<?=site_url("master/tambahvariasi")?>" method="POST">
