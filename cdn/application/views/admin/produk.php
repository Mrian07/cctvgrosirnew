...
<a href="<?=site_url("master/produkform")?>" class="btn btn-success float-right m-r-10"><i class="fas fa-plus-circle"></i> Produk Baru</a>
...
<script type="text/javascript">
	...
	function refreshTabel(page){
		...
		$.post("<?=site_url("master/produk?load=true")?>&page="+page+"&perpage="+perpage,{"cari":$("#cari").val(),"status":$("#status").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
				...
				$.post("<?=site_url("api/hapusproduk")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					...
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("master/produk")?>";
						});
					}else{
